...
namespace Instalog
{
    namespace detail
    {
        // Selects either the value "left" or the value "right" depending on the value of useRight
        template <typename T, T left, T right, bool useRight>
        struct MaxImpl
            : public std::integral_constant<T, left> // use the left value
        {
        };

        // when useRight is true.
        template <typename T, T left, T right>
        struct MaxImpl<T, left, right, true>
            : public std::integral_constant<T, right> // use the right value
        {
        };
        ...
        template <typename T, T left, T right>
        struct Max
            : public MaxImpl<T, left, right, (left < right)>
        {
        };
    }
    ...
    class OptimisticBuffer
    {
        ...
        typedef typename detail::Max<std::size_t, std::alignment_of<unsigned char*>::value, AlignmentValue>::type AlignmentType;
        // The size of the stack allocated region.
        typedef typename detail::Max<std::size_t, sizeof(unsigned char*), SizeGuess>::type SizeType;
        // The type of stack allocation region.
        typedef typename std::aligned_storage<SizeType::value, AlignmentType::value>::type StorageType;
    ...
    public:
        ...
        /**
         * Initializes a new instance of the OptimisticBuffer class.
         * @tparam std::size_t OtherSizeGuess The size guess for the OptimisticBuffer to copy.
         * @tparam std::size_t OtherAlignmentValue The alignment of the OptimisiticBuffer to copy.
         * @param toCopy The OptimisticBuffer to copy.
         */
        template <std::size_t OtherSizeGuess, std::size_t OtherAlignmentValue>
        OptimisticBuffer(OptimisticBuffer<OtherSizeGuess, OtherAlignmentValue> const& toCopy)
            : currentSize(toCopy.currentSize)
        {
            static_assert(OptimisticBuffer<OtherSizeGuess, OtherAlignmentValue>::AlignmentType::value > AlignmentType::value,
                "Tried to copy from a more aligned memory block to a less aligned memory block.");
         ...
         * @tparam std::size_t OtherSizeGuess The size guess for the OptimisticBuffer to copy.
         * @tparam std::size_t OtherAlignmentValue The alignment of the OptimisiticBuffer to
         *  copy.
         * @param toCopy The OptimisticBuffer to copy.
         * @return *this
         */
        template <std::size_t OtherSizeGuess, std::size_t OtherAlignmentValue>
        OptimisticBuffer<SizeGuess, AlignmentValue>& operator=(OptimisticBuffer<OtherSizeGuess, OtherAlignmentValue> const& toCopy)
            : currentSize(toCopy.currentSize)
        {
            static_assert(OptimisticBuffer<OtherSizeGuess, OtherAlignmentValue>::AlignmentType::value > AlignmentType::value,
                "Tried to copy assign from a more aligned memory block to a less aligned memory block.");
            if (GetPointer() != toCopy.GetPointer()) // Handle assignment to self.
            {
                if (Size() != toCopy.Size()) // Optimization -- if sizes match no need to reallocate.
                {
                    Destroy();
                    Allocate(toCopy.Size());
                }
