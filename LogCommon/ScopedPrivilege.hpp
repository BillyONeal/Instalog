...
class ScopedPrivilege : boost::noncopyable
{
    ...
    ScopedPrivilege(LPCWSTR privilegeName) : hThisProcess(::GetCurrentProcess())
    {

        if (::OpenProcessToken(
                hThisProcess, TOKEN_ADJUST_PRIVILEGES, &hThisProcessToken) == 0)
        {
        ...
        if (::LookupPrivilegeValueW(
                nullptr, privilegeName, &privileges.Privileges[0].Luid) == 0)
        {
            Win32Exception::ThrowFromLastError();
        }
        if (::AdjustTokenPrivileges(
                hThisProcessToken, FALSE, &privileges, 0, nullptr, nullptr) ==
            0)
        {
            Win32Exception::ThrowFromLastError();
        }
