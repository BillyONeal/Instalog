...
struct uninitalized_expected : public std::logic_error
{
    ...
    uninitalized_expected()
        : std::logic_error(
              "Attempted to read an expected<t> which was unintialized.")
    {
    }
};

template <typename Ty> class expected;

template <typename Ty> inline void swap(expected<Ty>& lhs, expected<Ty>& rhs);

/**
 * An expected of T. Contains either a T or an exception if an exception was
 * encountered when producing T.
 * 
 * @remarks This class is loosely based on Andrei Alexandrescu's talk,
 * "Systematic Error Handling in C++". See
 *          http://channel9.msdn.com/Shows/Going+Deep/C-and-Beyond-2012-Andrei-Alexandrescu-Systematic-Error-Handling-in-C
 * @tparam Ty Type of the contained object.
 */
template <typename Ty> class expected
{
    static_assert(std::is_nothrow_destructible<Ty>::value,
                  "expected<t> requires that Ty be nothrow destructable.");
    ...
    private:
    union
    { // In C++11, one is allowed to have a union with a type with a
      // constructor. But MSVC++ doens't support that
        // as of this writing.
        typename std::aligned_storage<sizeof(Ty),
                                      std::alignment_of<Ty>::value>::type
        tStorage;
        std::aligned_storage<sizeof(std::exception_ptr),
    ...
    /**
     * Initializes a new instance of the expected class. Requires that Ty be
     * copy constructable.
     * @param source The actual expected item.
     */
    expected(const_reference source) : isValid(true)
    {
     ...
     */
    expected(Ty&& source) : isValid(true)
    {
        new (&get_value_storage()) Ty(std::move(source));
    }

    /**
     * Initializes a new instance of the expected class from an existing copy.
     * Requires Ty to be copy constructable.
     * @param source Source expected<T> to copy.
     */
    expected(expected<Ty> const& source) : isValid(source.isValid)
    {
        if (source.isValid)
        {
        ...
        {
            new (&get_exception_storage()) std::exception_ptr(
                source.get_exception_storage());
        }
    }

    /**
     * Move initializes a new instance of the expected class from an existing
     * copy. Requires Ty to be move constructable.
     * @param source Source expected<T> to move from.
     */
    expected(expected<Ty>&& source) : isValid(source.isValid)
    {
        if (source.isValid)
        {
            new (&get_value_storage())
                Ty(std::move(source.get_value_storage()));
        }
        else
        {
            new (&get_exception_storage()) std::exception_ptr(
                std::move(source.get_exception_storage()));
        }
    }
     ...
     * @param ptr The exception pointer from which this instance shall be
     * initialized.
     ...
     * @tparam typename E Type of the exception. This parameter should not be
     * explictly specified.
     * @param [in,out] ex The exception from which the new instance shall be
     * constructed.
    ...
    /**
     * Query if this instance is valid. Valid means that the instance contains a
     * T, rather than an exception.
     * 
    ...
    /**
     * Rethrows this instance if an exception is contained herein. If this
     * instance is valid, does nothing.
     * @exception uninitalized_expected Thrown when this instance was default
     * constructed.
     */
    void rethrow() const
    {
     ...
     * @exception Unspecified The exception contained herein if this instance is
     * not valid and was constructed with an exception.
     * @exception uninitialized_expected if this instance was default
     * constructed.
    ...
    void swap(expected<Ty>& target)
    {
        if (isValid)
        {
            ...
            else
            {
                ...
                using std::exception_ptr;
                std::exception_ptr moved(
                    std::move(target.get_exception_storage()));
                target.get_exception_storage().~exception_ptr();
                new (&target.get_value_storage())
                    Ty(std::move(get_value_storage()));
                get_value_storage().~Ty();
