...
class RegistryKeySizeInformation
{
    ...
    /// @brief    Constructor.
    ///
    /// @param    lastWriteTime      Time of the last write to the indicated
    /// registry key.
    /// @param    numberOfSubkeys    Number of sub keys of this registry key.
    /// @param    numberOfValues     Number of values in this registry key.
    RegistryKeySizeInformation(std::uint64_t lastWriteTime,
                               std::uint32_t numberOfSubkeys,
                               std::uint32_t numberOfValues);
...
class RegistryKey : boost::noncopyable
{
    ...
    /// @brief    Opens a registry key.
    ///
    /// @param    parent          The parent key, where the key to open is
    /// rooted.
