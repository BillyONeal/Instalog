...
\item[Output Description] \hfill \\
\var{GeneratedName} shall consist of \var{Name} if it's
contents are not blank, followed by \var{AlternateName} if it's contents are
...
\item[Output Description] \hfill \\
\var{Providers} is a whitespace or comma seperated list of values. A seperate
...
``Move'' shall generate a fix quarentining the affected file.
\end{description}
...
paths of some importantant DLLs. One of these is the ``Server'' DLL, which
...
script quarentining the indicated DLL.
\end{description}
...
``Move'' shall generate a script quarentining the indicated file.
\end{description}
...
loaded instead of the indiciated binary. Malware uses this to redirect
...
Image File Execution Options key. ``Move'' shall generate a script quarentining
the indiciated \var{Target}.
\end{description}
