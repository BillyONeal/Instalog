...
The Load Points machine report contains settings which affect the entire
machine. The first line of the machine report indicates the machine name, and is
of the form
\begin{verbatim}
Identity: [${MachineName}] MACHINE
\end{verbatim}
where \var{MachineName} is the computer name of the machine, escaped using the
general escaping function defined in \ref{generalescape} with an escape
character hash (\verb|#|) and a right delimiter character right bracket
(\verb|]|).
...
The load points report can be invoked in a script by calling
\verb|:LoadPoints|.
...
\ref{generalescape}, using an escape character of hash (\verb|#|), and a right
delimiter of space (\verb|' '|) \var{CLSID} is escaped using the default
escaping function defined in \ref{generalescape}, using an escape character of hash
...
\ref{generalescape}, using an escape character of hash (\verb|#|), and a right
delimiter of right square bracket (\verb|]|). \var{Target} shall be displayed
using the default file display format defined in \ref{stdfile}.
\item[Whitelisting Considerations] \hfill \\
None at this time.
\item[Fix Considerations] \hfill \\
...
\item[Data Sources] \hfill \\
The location of the TCP/IP database in Windows is stored in the registry at
\begin{verbatim}
[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters]
"DataBasePath"="${Path}"
\end{verbatim}

The hosts file is located at \var{Path}\verb|\Hosts|, typically
\verb|%SystemRoot%\system32\drivers\etc\hosts|.
...
\item[Log Format] \hfill \\
\verb|HostsFile: ${Path}\Hosts| \\
\verb|Hosts: ${ip} ${hostname}|
\item[Output Description] \hfill \\
\verb|${Path}\Hosts| shall be escaped using the general escape function defined
in \ref{generalescape}.

The variable \var{hostname} shall be escaped using the URL escaping scheme
defined in \ref{urlescape}, where the escape character is the hash mark (\verb|#|).
\item[Whitelist Considerations] \hfill \\
