...
receives in the list \var{response\textunderscore ip}.  For each of the
\var{response\textunderscore ip} IP addresses in \var{response\textunderscore
ips}, Instalog shall perform a DNS query against a safe DNS server (such as
Google's DNS servers or OpenDNS's DNS Servers) to get the url associated with
that IP (referred to as \var{response\textunderscore url}).  If the response url
does not match the expected response, then the result shall be logged according
to the format explained in the \textbf{Log Format} section.
...
\var{response\textunderscore url} shall be set to ``no response'' and shall be
logged according to the \textbf{Log Format} section.
\item[Log Format] \hfill
\vspace{-\baselineskip}
\begin{verbatim}
SpoofedDNS: ${url} ${response_ip} ${response_url}
\end{verbatim}
\item[Output Description] \hfill \\
\var{url} and \var{response\textunderline url} shall be escaped according to the
