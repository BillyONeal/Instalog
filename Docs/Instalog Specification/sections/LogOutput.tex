...
querying functions like \verb|VerQueryValue|):
\begin{verbatim}
...
Finally, if some failure occured in path resolution that was not the file not
...
\var{State} is one of the following:
\begin{description}
...
\var{Start} is one of the following:
\begin{description}
...
\end{itemize}
and the following additional directories on machines as old or more recent
than Windows Vista:
\begin{itemize}
...
\subsection{Find3M}
Find3M is taken from sUBs' DDS, almost without modification. It originally stood
for ``Find files created in the last 3 months'', but has since been expanded to
...
\begin{enumerate}
    ...
    \var{List2}.
    \begin{itemize}
        \item Windows Directory \textbackslash System
